              ...
              <table class="table table-bordered">
                ...
                <tbody id="tabelacesta">
                  ...
                  <tr>
                    ...
                    @else
                    <td class="text-center col-md-1"><a href="#"><img src="{{url('images/product/tarja_vermelha.jpg')}}" alt="iPod Classic" title="iPod Classic"></a></td>
                    @endif
                    <td class="text-left"><a href="{{ route('perfilproduto',['idproduto'=>$cestas->produtofk,'idcliente'=>$id]) }}">{{$cestas->nome}}</a></td>
                    <td class="text-left">{{$cestas->tipo}}</td>
                    <td class="text-left">
                      <div style="max-width: 200px;" class="input-group btn-block">
                      <input type="hidden" class="id" value="{{$cestas->id}}">
                       <input type="hidden" class="descontor" value="{{$cestas->desconto}}">
                       <input type="hidden" class="preco" value="{{$cestas->preco}}">
                        <input type="text" class="form-control quantidade" size="1" value="{{$cestas->quantidade}}" name="quantity">
                        <span class="input-group-btn">
                    <!-- <button class="btn quantidade" title="" data-toggle="tooltip" data-original-title="Atualizar"><i class="fa fa-refresh"></i></button> -->
                    <button class="btn btn-danger" onclick="Excluircesta({{$cestas->id}})" title="" data-toggle="tooltip" type="button" data-original-title="Remove"><i class="fa fa-times-circle"></i></button>
                    </span></div>
                    </td>
                    <td class="text-right">R$:{{number_format((((($cestas->desconto/100)*$cestas->preco) - $cestas->preco)*(-1)), 2, ',', ' ')}}</td>
                    <td class="text-right">R$:{{number_format($cestas->total, 2, ',', ' ')}}</td>
                  </tr>
                  @endforeach
